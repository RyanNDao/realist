...
from helpers.database_helpers import build_dynamic_update_query_template
...
class TruliaHouseListingDAO():
    ...
    def getAllListings(self, tableName=constants.TRULIA_MAIN_TABLE_NAME) -> list:
        self.cursor.execute(f'SELECT * FROM {tableName}')
        rows = self.cursor.fetchall()
        LOGGER.info(f'getAllListings returned {len(rows)} total results')
        return rows
        
    def insertListingIntoTable(self, truliaHouseListing: TruliaHouseListing, columns=constants.TRULIA_MAIN_TABLE_VALUES, tableName=constants.TRULIA_MAIN_TABLE_NAME) -> None:
    ...
    def getListingByKey(self, keyValue: str, keyName='key', tableName=constants.TRULIA_MAIN_TABLE_NAME) -> dict:
        self.cursor.execute(f'SELECT * FROM {tableName} WHERE {keyName} = %s', (keyValue, ))
        return self.cursor.fetchone()
    
    def deleteListingByKey(self, key: str, tableName=constants.TRULIA_MAIN_TABLE_NAME) -> None:
    ...
    def deleteListingByKeyUsingDataObject(self, truliaHouseListing: TruliaHouseListing, tableName=constants.TRULIA_MAIN_TABLE_NAME) -> None:
    ...
    def updateListingEntryInTable(self, truliaHouseListing: TruliaHouseListing, tableName=constants.TRULIA_MAIN_TABLE_NAME, keyName = 'key'):
        setQueryTemplate = build_dynamic_update_query_template(constants.TRULIA_MAIN_TABLE_COLUMNS, keyName=keyName)
